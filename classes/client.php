...
class client{
    ...
    function getRequest_group(){
        return $this->request_group;
    }
    ...
    function setID($ID){
        ...
        global $con;
        $stmt = $con->prepare("SELECT * FROM `user` WHERE `ID`=$ID AND `type`='client'");
        $stmt->execute();
        $data= $stmt->fetch();
  ...
  // setters
    ...
    function saveToDB(){
     ...
     // update data
        ...
        ,`email`='$email',`image`='$image',`register Date`='$registerDate',
    ...
    function addComment($comment,$postID){
        ...
        global $con;
        $stmt = $con->prepare("INSERT INTO `comment`(`comment-rating`, `creator ID`, `post ID`, `type`) 
                               VALUES ('$comment','$this->ID','$postID','comment')");
        $stmt->execute();
        return $stmt;
